#![deny(missing_docs)]
#![doc(html_root_url = "https://docs.rs/lapin-futures-tls-internal/0.1.1/")]
...
pub trait AMQPConnectionTlsExt<TlsStream: AsyncRead + AsyncWrite + Send + Sync + 'static, F: FnOnce(io::Error) + Send + 'static> {
    /// Method providing a `lapin_futures::client::Client` wrapped in a `Future`
    fn connect<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(self, heartbeat_error_handler: F, connector: Connector) -> Box<Future<Item = lapin::client::Client<AMQPStream<TlsStream>>, Error = io::Error> + Send + 'static>;
    /// Method providing a `lapin_futures::client::Client` and `lapin_futures::client::HeartbeatHandle` wrapped in a `Future`
    fn connect_cancellable<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(self, heartbeat_error_handler: F, connector: Connector) -> Box<Future<Item = (lapin::client::Client<AMQPStream<TlsStream>>, lapin::client::HeartbeatHandle), Error = io::Error> + Send + 'static>;
}

impl<TlsStream: AsyncRead + AsyncWrite + Send + Sync + 'static, F: FnOnce(io::Error) + Send + 'static> AMQPConnectionTlsExt<TlsStream, F> for AMQPUri {
    fn connect<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(self, heartbeat_error_handler: F, connector: Connector) -> Box<Future<Item = lapin::client::Client<AMQPStream<TlsStream>>, Error = io::Error> + Send + 'static> {
...
impl<'a, TlsStream: AsyncRead + AsyncWrite + Send + Sync + 'static, F: FnOnce(io::Error) + Send + 'static> AMQPConnectionTlsExt<TlsStream, F> for &'a str {
    fn connect<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(self, heartbeat_error_handler: F, connector: Connector) -> Box<Future<Item = lapin::client::Client<AMQPStream<TlsStream>>, Error = io::Error> + Send + 'static> {
...
impl<TlsStream: AsyncRead + AsyncWrite + Send + 'static> AMQPStream<TlsStream> {
    fn from_amqp_uri<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(uri: &AMQPUri, connector: Connector) -> Box<Future<Item = Self, Error = io::Error> + Send + 'static> {
        match uri.scheme {
    ...
    fn tls<Connector: FnOnce(String, TcpStream) -> Box<Future<Item = Box<TlsStream>, Error = io::Error> + Send + 'static> + Send + 'static>(host: String, port: u16, connector: Connector) -> Box<Future<Item = Self, Error = io::Error> + Send + 'static> {
        Box::new(
            open_tcp_stream(host.clone(), port).and_then(move |stream| {
                connector(host, stream).map(AMQPStream::Tls)
            })
        )
    }
}
