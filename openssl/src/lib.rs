#![deny(missing_docs)]
#![doc(html_root_url = "https://docs.rs/lapin-futures-openssl/0.3.0/")]
...
pub trait AMQPConnectionOpensslExt<F: FnOnce(io::Error) + Send + 'static>: AMQPConnectionExt<F> where Self: Sized {
    ...
    fn connect_cancellable(self, heartbeat_error_handler: F) -> Box<Future<Item = (lapin::client::Client<AMQPStream>, lapin::client::HeartbeatHandle), Error = io::Error> + Send + 'static> {
        AMQPConnectionExt::connect_cancellable::<tls_api_openssl::TlsConnector>(self, heartbeat_error_handler)
    }
}
