#![deny(missing_docs)]
#![doc(html_root_url = "https://docs.rs/lapin-futures-openssl/0.5.0/")]
...
/// Represents either a raw `TcpStream` or a `SslStream` backend by `tokio-openssl`.
/// The `SslStream` is wrapped in a `Box` to keep the enum footprint minimal.
pub enum AMQPStream {
    /// The raw `TcpStream` used for basic AMQP connections.
    Raw(TcpStream),
    /// The `SslStream` used for AMQPs connections.
    Tls(Box<SslStream<TcpStream>>),
}

/// Add a connect method providing a `lapin_futures::client::Client` wrapped in a `Future`.
pub trait AMQPConnectionOpensslExt<F: FnOnce(io::Error) + Send + 'static> {
    ...
    fn tls(host: String, port: u16) -> Box<Future<Item = Self, Error = io::Error> + Send + 'static> {
        Box::new(
            open_tcp_stream(host.clone(), port).join(
                futures::future::result(SslConnector::builder(SslMethod::tls()).map_err(|_| io::Error::new(io::ErrorKind::Other, "Failed to create connector")))
            ).and_then(move |(stream, connector)| {
                connector.build().connect_async(&host, stream).map_err(|_| io::Error::new(io::ErrorKind::Other, "Failed to connect")).map(Box::new).map(AMQPStream::Tls)
            })
        )
    }
}
