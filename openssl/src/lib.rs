...
pub trait AMQPConnectionOpensslExt: AMQPConnectionExt {
    ...
    fn connect<F: FnOnce(io::Error) + 'static>(&self, handle: Handle, heartbeat_error_handler: F) -> Box<Future<Item = lapin::client::Client<AMQPStream>, Error = io::Error> + 'static> {
        AMQPConnectionExt::connect::<tls_api_openssl::TlsConnector, _>(self, handle, heartbeat_error_handler)
    }
}

impl AMQPConnectionOpensslExt for AMQPUri {}
impl AMQPConnectionOpensslExt for str {}
